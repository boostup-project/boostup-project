...
export const getStaticProps = async () => {
  const res = await fetch("https://dc23-182-226-233-7.jp.ngrok.io/lesson");
...
const Card = ({ cards }) => {
  ...
  const { refetch, data, status, isSuccess } = useQuery(
    "cards",
    () => getMainCard(),
    { initialData: props.cards },
  );
      ...
      <div className="flex flex-col w-full h-1/4 border border-borderColor rounded-lg">
        ...
        <div className="flex flex-col w-full h-2/3">
          <div className="flex flex-row whitespace-wrap">
            <div className="flex justify-center items-center px-1 py-0.5 ml-1 my-1 border rounded-xl bg-[#0A83F2] desktop:text-xs tablet:text-[10px] text-[6px]"></div>
            {card.lang}
          </div>
          <div className="flex justify-start items-start w-full h-fit font-SCDream5 desktop:text-xs tablet:text-[10px] text-[8px] text-textColor ml-2  my-1"></div>
          <div className="flex inline-block justify-start items-start w-full h-fit font-SCDream6  desktop:text-base tablet:text-sm text-xs text-textColor ml-2 mb-2 whitespace-wrap "></div>
