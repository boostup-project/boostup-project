...
const BasicInfo = () => {
  ...
  const onSubmit = (e: BasicInfo) => {
    if (
      ...
      const { title, company, career, cost } = e;
      const baseSave = {
        profileImage: previewImg,
        title,
        language: checkedList,
        company,
        career,
    ...
    if (previewImg.length === 0) {
      setOptionErr(prev => ({ ...prev, previewImg: "필수 정보입니다" }));
    }
    if (checkedList.length === 0) {
      setOptionErr(prev => ({ ...prev, checkErr: "필수 정보입니다" }));
    }
    if (selectedOptions.length === 0) {
      setOptionErr(prev => ({ ...prev, selectErr: "필수 정보입니다" }));
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-row justify-center items-center w-full h-fit mt-10">
          <SmallBtn onClick={e => toWrite(e)}>취 소</SmallBtn>
